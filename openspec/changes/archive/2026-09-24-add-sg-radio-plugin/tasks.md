...
- [x] 8.2 On this machine, after group 7: install, open the popup card, play a station from its list, change volume with the slider and by scroll, pause, stop, and uninstall; verify each spec scenario in `openspec/changes/add-sg-radio-plugin/specs/` holds and run `openspec validate add-sg-radio-plugin`
